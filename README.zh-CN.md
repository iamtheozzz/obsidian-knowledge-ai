...
**agent 做不了的最后一步**：打开 Obsidian →「设置 → 第三方插件」→ 启用 **Lark Knowledge AI**。
...
| 点左侧栏的图标 | 同上 |
| 命令面板 → `Lark Knowledge AI: 提问` | 同上 |

输入问题，回车。答案流式输出，末尾附参考资料。

`Enter` 提问 · `Shift + Enter` 换行 · `Esc` 中止生成（再按一次关窗）

> 插件不预设快捷键。想要的话到「设置 → 快捷键」搜 `Lark Knowledge AI` 自己绑一个。
